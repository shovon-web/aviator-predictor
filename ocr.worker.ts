import * as Tesseract from 'tesseract.js';

let scheduler: Tesseract.Scheduler | null = null;
let isInitialized = false;

const initializeTesseract = async () => {
    if (isInitialized) return;

    console.log('OCR Worker: Initializing Tesseract...');
    scheduler = Tesseract.createScheduler();
    const worker = await Tesseract.createWorker('eng', 1, {
        logger: m => console.log(m.status === 'recognizing text' ? `OCR: ${(m.progress * 100).toFixed(0)}%` : m.status)
    });
    // Fine-tune Tesseract parameters for better accuracy
    await worker.setParameters({
        // Whitelist only the characters we expect
        tessedit_char_whitelist: '0123456789.x',
        // Treat the image as a single line of text, which is ideal for this use case
        tessedit_pageseg_mode: Tesseract.PSM.SINGLE_LINE,
    });
    scheduler.addWorker(worker);
    isInitialized = true;
    console.log('OCR Worker: Tesseract initialized.');
    self.postMessage({ type: 'INITIALIZED' });
};

self.onmessage = async (event: MessageEvent<{ type: string; payload?: ImageData }>) => {
    const { type, payload } = event.data;

    if (type === 'INIT') {
        await initializeTesseract();
        return;
    }

    if (type === 'TERMINATE') {
        await scheduler?.terminate();
        scheduler = null;
        isInitialized = false;
        console.log('OCR Worker: Tesseract terminated.');
        self.postMessage({ type: 'TERMINATED' });
        return;
    }

    if (type === 'PROCESS_FRAME' && payload) {
        if (!scheduler || !isInitialized) {
            console.warn('OCR Worker: Tesseract not initialized. Ignoring frame.');
            return;
        }

        try {
            // FIX: Tesseract's scheduler.addJob expects an ImageLike object, but ImageData is not assignable to ImageLike.
            // To fix this, we create an OffscreenCanvas, put the ImageData onto it, and then pass the canvas to Tesseract.
            const canvas = new OffscreenCanvas(payload.width, payload.height);
            const ctx = canvas.getContext('2d');
            if (!ctx) {
                throw new Error('Failed to get 2D context from OffscreenCanvas');
            }
            ctx.putImageData(payload, 0, 0);

            const result = await scheduler.addJob('recognize', canvas);
            const text = result.data.text;
            
            // A more robust regex to find patterns like "1.23x", "10.5x", "5x" etc.
            // It looks for digits, allows for optional dots, and ends with 'x' (case-insensitive).
            const multiplierRegex = /(\d[\d.]*)\s*x/gi;
            let matches;
            const values = [];

            // Loop through all found matches in the recognized text
            while ((matches = multiplierRegex.exec(text)) !== null) {
                // The first capture group contains the number string
                const num = parseFloat(matches[1]);
                if (!isNaN(num)) {
                    values.push(num);
                }
            }
            
            if (values.length > 0) {
                // Find the largest valid number among matches. This helps filter out
                // other numbers on the screen (e.g., balances, bet amounts).
                const highestValue = Math.max(...values);
                self.postMessage({ type: 'RESULT', payload: highestValue });
            }
        } catch (error) {
            console.error('OCR Worker: Error during recognition:', error);
            self.postMessage({ type: 'ERROR', payload: 'Recognition failed' });
        }
    }
};
