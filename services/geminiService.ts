...
// FIX: Initialize with process.env.API_KEY directly as per coding guidelines.
...
export const getPrediction = async (history: Multiplier[]): Promise<Prediction> => {
  ...
  // --- Start of advanced analysis logic ---
  ...
  // --- End of advanced analysis logic ---
  ...
  try {
    ...
    // FIX: Access response text correctly via `response.text` and parse it.
    // The sanitization logic is kept for robustness against models adding markdown fences.
