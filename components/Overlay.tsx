...
import { Multiplier, Prediction, RangeCategory, Trend } from '../types';
...
const Overlay: React.FC = () => {
  ...
  const { isCapturing, startCapture, stopCapture, error: captureError } = useScreenCapture(processFrame);

  // Effect to fetch prediction when history changes
  useEffect(() => {
    if (history.length >= 5) {
      setIsLoading(true);
  ...
  // Effect to calculate trend from history
  useEffect(() => {
    if (history.length < 4) {
    ...
    const recentValues = history.slice(0, 4).map(h => h.value).reverse(); // Oldest to newest
    const [v1, v2, v3, v4] = recentValues; // v1 is oldest, v4 is newest

    const isStrictlyIncreasing = v4 > v3 && v3 > v2;
    const isStrictlyDecreasing = v4 < v3 && v3 < v2;
    ...
    } else if (range > 4.0) { // Large swings indicate volatility
      setTrend(Trend.VOLATILE);
    } else if (range < 1.5) { // Small swings indicate stability
  ...
  // Effect to handle capture errors
  useEffect(() => {
    if (captureError) {
  ...
  return (
    <div
      ...
      style={{ top: position.y, left: position.x, transform: isMinimized ? 'translateY(calc(100% - 32px))' : 'translateY(0)'}}
    >
      <Header
