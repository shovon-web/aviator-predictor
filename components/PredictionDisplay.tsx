...
const PredictionDisplay: React.FC<PredictionDisplayProps> = ({ prediction, t, isLoading, historyLength, trend }) => {
  const { patternAnalysis, nextRoundPrediction, probabilities, cashOutTarget, investmentAdvice } = prediction;
  ...
  const calculateConfidence = () => {
    ...
    // 1. Data Score (0 to 1) - confidence grows with more data, maxing out at 30 rounds
    const dataScore = Math.min(historyLength / 30, 1.0);

    // 2. Probability Spread Score (0 to 1) - higher confidence if one probability is much larger than others
    // FIX: Explicitly convert probability values to numbers to prevent type errors during arithmetic operations.
    const probs = Object.values(prediction.probabilities).map(Number).sort((a, b) => b - a); // [high, medium, low]
    const probSpread = (probs[0] - probs[1]) / 100; // Difference between the top two probabilities, normalized
  ...
  const getTrendIndicator = () => {
    switch (trend) {
      case Trend.INCREASING:
        return {
          icon: <TrendUpIcon />,
          labelKey: 'trendIncreasing',
          badgeColor: 'bg-green-500/20 text-green-300',
        };
      case Trend.DECREASING:
        return {
          icon: <TrendDownIcon />,
          labelKey: 'trendDecreasing',
          badgeColor: 'bg-red-500/20 text-red-400',
        };
      case Trend.VOLATILE:
        return {
          icon: <ZapIcon />,
          labelKey: 'trendVolatile',
          badgeColor: 'bg-yellow-500/20 text-yellow-300',
        };
      case Trend.STABLE:
        return {
          icon: <StableTrendIcon />,
          labelKey: 'trendStable',
        ...
        <div className={`bg-slate-800/50 p-4 rounded-lg text-center border-t-4 transition-colors duration-500 ${confidence.color}`}>
            <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center space-x-2">
                    {trendIndicator && (
                        <div
                            className={`flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${trendIndicator.badgeColor}`}
                            title={t(trendIndicator.labelKey)}
                        >
                            {trendIndicator.icon}
